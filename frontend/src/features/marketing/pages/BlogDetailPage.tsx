  ...
  if (isError || !post) {
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 py-24 px-4 max-w-2xl mx-auto text-center space-y-4">
        ...
        <p className="text-slate-400">The blog post you are looking for does not exist or has been archived.</p>
        <Link
          to="/blog"
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 py-16 px-4 sm:px-6 lg:px-8">
      <SeoHead
        ...
        breadcrumbSchema={{
          ...
          itemListElement: [
            ...
            { '@type': 'ListItem', position: 3, name: post.title, item: `https://trashhere.com/blog/${post.slug}` },
          ],
        }}
      />
      ...
      <article className="max-w-4xl mx-auto space-y-10">
        ...
        {/* Cover Image */}
        {post.coverImage && (
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="rounded-3xl overflow-hidden border border-slate-800 shadow-2xl">
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="pt-8 border-t border-slate-800 flex flex-wrap gap-2">
            <span className="text-xs text-slate-500 font-semibold uppercase tracking-wider py-1">Tags:</span>
            {post.tags.map((tag) => (
              <span key={tag} className="text-xs px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-slate-300">
                #{tag}
              </span>
            ))}
          </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-8 sm:p-12 text-center space-y-4 my-12">
          ...
          <p className="text-sm text-slate-400 max-w-xl mx-auto">
            Experience our SHA-256 carbon offset engine and algorithmic dispatch routing live in your city.
          </p>
          <div className="pt-2">
            <Link
              to="/app"
