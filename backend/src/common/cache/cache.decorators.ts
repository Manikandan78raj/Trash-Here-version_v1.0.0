...
export function Cacheable(options: CacheableOptions): MethodDecorator {
  return function (target: any, propertyKey: string | symbol, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      const cacheService = (this as any).cacheService || (this as any).redisCacheService;
      ...
      return cacheService.wrap(cacheKey, () => originalMethod.apply(this, args), options.ttl);
    };
    return descriptor;
  };
}
      ...
      if (cacheService) {
        if (options.pattern) {
          ...
          if (typeof cacheService.del === 'function') {
            const argsHash = args.length ? ':' + JSON.stringify(args) : '';
            await cacheService.del(`th:cache:${options.keyPrefix}:${String(propertyKey)}${argsHash}`);
          }
        }
      }
      return result;
