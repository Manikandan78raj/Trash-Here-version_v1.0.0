...
describe("RedisCacheService (TDD Unit Suite)", () => {
  ...
  it("should delete multiple keys by wildcard pattern (delByPattern)", async () => {
    await service.set(
      "th:cache:prod:marketing:stats:global",
      { visits: 100 },
      60,
    );
    await service.set(
      "th:cache:prod:marketing:blog:post-1",
      { title: "Post 1" },
      60,
    );
    await service.set(
      "th:cache:prod:wallet:balance:user-1",
      { balance: 50 },
      60,
    );

    const deletedCount = await service.delByPattern(
      "th:cache:prod:marketing:*",
    );
    expect(deletedCount).toBe(2);

    expect(
      await service.get("th:cache:prod:marketing:stats:global"),
    ).toBeNull();
    expect(await service.get("th:cache:prod:marketing:blog:post-1")).toBeNull();
    expect(await service.get("th:cache:prod:wallet:balance:user-1")).toEqual({
      balance: 50,
    });
  });
