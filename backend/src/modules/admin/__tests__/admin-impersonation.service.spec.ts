...
describe('AdminImpersonationService (TDD Suite)', () => {
  ...
  describe('startImpersonation', () => {
    it('2. should verify target user is not SUPER_ADMIN, create log, emit audit event, and return token', async () => {
      ...
      mockPrismaService.adminImpersonationLog.create.mockResolvedValue(mockImpersonationLog);

      const result = await service.startImpersonation(
        'usr-admin-1',
        { targetUserId: 'usr-target-1', reason: 'Ticket #8891 - Wallet topup error' },
        '10.0.0.1',
        'Mozilla/5.0'
      );
    ...
    it('3. should throw ForbiddenException when attempting to impersonate a SUPER_ADMIN', async () => {
      ...
      await expect(
        service.startImpersonation('usr-admin-1', { targetUserId: 'usr-super-1', reason: 'Audit' })
    ...
    it('4. should throw NotFoundException when target user does not exist', async () => {
      ...
      await expect(
        service.startImpersonation('usr-admin-1', { targetUserId: 'non-existent', reason: 'Test' })
  ...
  describe('stopImpersonation', () => {
    it('5. should update endedAt timestamp and emit audit event', async () => {
      mockPrismaService.adminImpersonationLog.findUnique.mockResolvedValue(mockImpersonationLog);
