...
describe('AdminFinanceService (TDD Suite)', () => {
  ...
  const mockMaterialBatches = [
    {
      ...
      purityPercent: 99.0,
      status: 'SOLD',
      category: { pricePerKg: 0.50 }, // 1000 * $0.50 = $500.00 recycler revenue
    },
  ];
  ...
  describe('calculatePnL', () => {
    it('2. should aggregate Stripe payments, collector payouts, points liabilities, and recycler revenues to calculate net margin', async () => {
      ...
      mockPrismaService.materialBatch.findMany.mockResolvedValue(mockMaterialBatches);
  ...
  describe('reconcileLedgers', () => {
    it('4. should verify wallet balances match sum of completed transactions without discrepancies', async () => {
      ...
      mockPrismaService.transaction.findMany.mockResolvedValue([
        { userId: 'usr-1', type: TransactionType.PAYMENT, status: TransactionStatus.COMPLETED, amount: 50.0, pointsAmount: 500 },
        { userId: 'usr-1', type: TransactionType.PAYOUT, status: TransactionStatus.COMPLETED, amount: 35.0, pointsAmount: 0 },
