...
describe('RecyclerProcessingService', () => {
  ...
  describe('startProcessing', () => {
    it('should start manufacturing process and allocate inventory', async () => {
      ...
      mockPrisma.materialBatch.update.mockResolvedValue({ ...mockBatch, status: BatchStatus.SHREDDING });
      mockPrisma.warehouseInventory.update.mockResolvedValue({ ...mockInventory, availableWeightKg: 3000 });
  ...
  describe('completeProcessing', () => {
    it('should complete processing and record output yield and waste loss', async () => {
      ...
      mockPrisma.materialBatch.update.mockResolvedValue({ ...mockBatch, status: BatchStatus.READY_FOR_SALE });
      mockPrisma.warehouseInventory.update.mockResolvedValue(mockInventory);

      const result = await service.completeProcessing('user-uuid-1', 'queue-uuid-1', {
        outputWeightKg: 1950,
        wasteLossKg: 50,
      });
