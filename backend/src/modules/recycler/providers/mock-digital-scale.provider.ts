...
import { IDigitalScaleProvider, ScaleReading } from '../interfaces/digital-scale.provider.interface';
...
export class MockDigitalScaleProvider implements IDigitalScaleProvider {
  ...
  private readonly secretKey = process.env.JWT_SECRET || 'trash-here-weighbridge-secret-key-2026';

  async getScaleReading(scaleId: string): Promise<ScaleReading> {
    this.logger.log(`Polling IoT weighbridge scale [${scaleId}] for stable weight reading...`);
    // Simulate realistic truck weight between 4,000 kg (tare) and 25,000 kg (gross)
    const simulatedWeight = Math.floor(Math.random() * (25000 - 4000 + 1)) + 4000;
    return {
