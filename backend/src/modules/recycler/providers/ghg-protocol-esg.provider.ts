...
} from "../interfaces/esg-report.provider.interface";
...
export class GhgProtocolEsgProvider implements IEsgReportProvider {
  ...
  private readonly multipliers: Record<
    string,
    { co2: number; energy: number; water: number }
  > = {
  ...
  calculateCarbonOffset(
    ...
    this.logger.debug(
      `Calculated GHG offsets for [${categorySlug || "default"}] (${netWeightKg}kg): CO2=${co2OffsetKg}kg, Energy=${energySavedKwh}kWh`,
    );
