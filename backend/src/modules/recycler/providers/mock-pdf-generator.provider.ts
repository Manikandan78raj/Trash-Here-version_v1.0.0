...
import {
  IPdfGeneratorProvider,
  ManifestTemplateDto,
  GeneratedPdfResult,
} from "../interfaces/pdf-generator.provider.interface";
...
export class MockPdfGeneratorProvider implements IPdfGeneratorProvider {
  ...
  async generateManifestPdf(
    templateData: ManifestTemplateDto,
  ): Promise<GeneratedPdfResult> {
    this.logger.log(
      `Generating legal PDF manifest [${templateData.manifestNumber}] of type [${templateData.manifestType}]...`,
    );
  ...
  async generateEsgReportPdf(reportData: any): Promise<GeneratedPdfResult> {
    this.logger.log(
      `Generating ESG Sustainability Report PDF for period [${reportData.reportingPeriod || "ANNUAL"}]...`,
    );
