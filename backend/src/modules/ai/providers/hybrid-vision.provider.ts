    ...
    try {
      ...
      const [yoloRes, reasoningRes] = await Promise.all([
        this.yoloProvider.detectWaste(imageUrl, options).catch((err) => {
          this.logger.warn(
            `YOLO localization failed in hybrid pipeline: ${err.message}`,
          );
          return null;
        }),
        this.openAiProvider
          .detectWaste(imageUrl, options)
          .catch(async (err) => {
            this.logger.warn(
            ...
            return this.geminiProvider
              ...
              .catch((geminiErr) => {
                this.logger.warn(
                  `Gemini fallback also failed: ${geminiErr.message}`,
                );
                return null;
              });
          }),
      ...
      if (!reasoningRes && !yoloRes) {
        throw new Error(
          "Both localization and reasoning providers failed in hybrid pipeline",
        );
      }
      ...
      const mergedObjects = yoloRes!.detectedObjects.map((obj, index) => {
        const reasoningObj =
          reasoningRes!.detectedObjects[index] ||
          reasoningRes!.detectedObjects[0];
        return {
          ...obj,
          materialType: reasoningObj
            ? reasoningObj.materialType
            : obj.materialType,
          isContaminant: reasoningObj
            ? reasoningObj.isContaminant
            : obj.isContaminant,
        };
      });
      ...
      this.logger.error(
        `Hybrid vision pipeline error: ${error.message}`,
        error.stack,
      );
      throw error;
    }
