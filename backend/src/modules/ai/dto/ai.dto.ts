...
export class AiJobStatusDto {
  ...
  @ApiPropertyOptional({ example: 450, description: 'Processing duration in ms' })
  processingMs?: number;

  @ApiPropertyOptional({ example: 'Model timeout', description: 'Error message if failed' })
  errorMessage?: string;
}
...
export class AiPredictionResponseDto {
  ...
  @ApiProperty({ example: 2.5, description: 'Contamination percentage by weight' })
  contaminationRate: number;

  @ApiProperty({ example: 0.94, description: 'Overall model confidence score' })
  overallConfidence: number;

  @ApiProperty({ example: AiRecommendationType.DIRECT_RECYCLE, enum: AiRecommendationType })
  recommendationType: AiRecommendationType;

  @ApiProperty({ example: 'Clean PET bottle detected. Ready for direct recycling.' })
  ...
  @ApiProperty({ description: 'Array of detected object bounding boxes and labels' })
