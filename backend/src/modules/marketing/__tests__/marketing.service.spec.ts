...
describe('MarketingService (TDD Suite)', () => {
  ...
  describe('submitContact', () => {
    ...
    it('should successfully create a contact submission when under rate limit', async () => {
      ...
      expect(mockPrismaService.contactSubmission.create).toHaveBeenCalledWith({
        data: expect.objectContaining({ status: 'PENDING', ipAddress: '192.168.1.100' }),
      });
    });
    ...
    it('should flag submission as SPAM when containing spam keywords', async () => {
      ...
      const spamDto = { ...contactDto, message: 'Buy bitcoin and viagra here!' };
    ...
    it('should throw 429 Too Many Requests when IP exceeded 5 submissions in last minute', async () => {
      ...
      await expect(service.submitContact(contactDto, '192.168.1.100')).rejects.toThrow(HttpException);
    });
  });
  ...
  describe('getBlogPosts & getBlogPostBySlug', () => {
    it('should return blog posts filtered by category', async () => {
      mockPrismaService.blogPost.findMany.mockResolvedValue([{ id: 'post-1', title: 'AI Routing' }]);
      const res = await service.getBlogPosts('Engineering');
      expect(res.success).toBe(true);
      expect(res.count).toBe(1);
      expect(mockPrismaService.blogPost.findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { isPublished: true, category: 'Engineering' } })
      );
    });

    it('should return post by slug', async () => {
      mockPrismaService.blogPost.findFirst.mockResolvedValue({ id: 'post-1', slug: 'ai-routing' });
    ...
    it('should throw NotFoundException if slug not found', async () => {
      ...
      await expect(service.getBlogPostBySlug('non-existent')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('createBlogPost', () => {
    it('should create blog post successfully', async () => {
      ...
      mockPrismaService.blogPost.create.mockResolvedValue({ id: 'post-1', slug: 'new-post' });
  ...
  describe('Careers & Applications', () => {
    it('should return career jobs', async () => {
      mockPrismaService.careerJob.findMany.mockResolvedValue([{ id: 'job-1', title: 'Rust Engineer' }]);
    ...
    it('should submit job application successfully', async () => {
      mockPrismaService.careerJob.findUnique.mockResolvedValue({ id: 'job-1', isPublished: true });
      mockPrismaService.careerApplication.findFirst.mockResolvedValue(null);
      mockPrismaService.careerApplication.create.mockResolvedValue({ id: 'app-101' });
    ...
    it('should throw BadRequestException if duplicate application exists', async () => {
      mockPrismaService.careerJob.findUnique.mockResolvedValue({ id: 'job-1', isPublished: true });
      mockPrismaService.careerApplication.findFirst.mockResolvedValue({ id: 'app-existing' });
  ...
  describe('SEO Metadata & Analytics', () => {
    ...
    it('should track analytics event', async () => {
      mockPrismaService.publicAnalyticsEvent.create.mockResolvedValue({ id: 'evt-1' });
      const res = await service.trackAnalyticsEvent({ eventName: 'VIEW', route: '/' }, '127.0.0.1');
      expect(res.success).toBe(true);
