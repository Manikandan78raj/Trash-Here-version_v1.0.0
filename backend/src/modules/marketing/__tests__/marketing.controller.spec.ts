...
describe("MarketingController (TDD Suite)", () => {
  ...
  it("should call submitContact on service", async () => {
    mockMarketingService.submitContact.mockResolvedValue({
      success: true,
      submissionId: "sub-1",
    });
  ...
  it("should call subscribeNewsletter on service", async () => {
    mockMarketingService.subscribeNewsletter.mockResolvedValue({
      success: true,
      subscriberId: "sub-1",
    });
  ...
  it("should call getBlogPosts on service", async () => {
    mockMarketingService.getBlogPosts.mockResolvedValue({
      success: true,
      count: 2,
      data: [],
    });
    ...
    expect(mockMarketingService.getBlogPosts).toHaveBeenCalledWith(
      "AI",
      "Routing",
    );
  });
  ...
  it("should call getBlogPostBySlug on service", async () => {
    mockMarketingService.getBlogPostBySlug.mockResolvedValue({
      success: true,
      data: { slug: "test" },
    });
  ...
  it("should call getCareerJobs on service", async () => {
    mockMarketingService.getCareerJobs.mockResolvedValue({
      success: true,
      count: 1,
      data: [],
    });
  ...
  it("should call applyForJob on service", async () => {
    mockMarketingService.applyForJob.mockResolvedValue({
      success: true,
      applicationId: "app-1",
    });
  ...
  it("should call getSeoMetadata on service", async () => {
    mockMarketingService.getSeoMetadata.mockResolvedValue({
      success: true,
  ...
  it("should call trackAnalyticsEvent on service", async () => {
    mockMarketingService.trackAnalyticsEvent.mockResolvedValue({
      success: true,
      eventId: "evt-1",
    });
