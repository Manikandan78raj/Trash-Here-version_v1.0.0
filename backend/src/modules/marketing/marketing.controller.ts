...
export class MarketingController {
  ...
  @ApiOperation({ summary: 'Submit public contact inquiry with spam detection and rate limiting' })
  @ApiResponse({ status: 201, description: 'Inquiry received and routed to enterprise team' })
  @ApiResponse({ status: 429, description: 'Rate limit exceeded (max 5 per minute per IP)' })
  ...
  @ApiOperation({ summary: 'Subscribe email to marketing newsletter with deduplication' })
  @ApiResponse({ status: 200, description: 'Successfully subscribed or reactivated' })
  async subscribeNewsletter(@Body() dto: SubscribeNewsletterDto) {
    return this.marketingService.subscribeNewsletter(dto);
  }

  @Get('blog')
  @ApiOperation({ summary: 'Get published blog posts with optional category or tag filter' })
  @ApiQuery({ name: 'category', required: false, example: 'Engineering & AI' })
  @ApiQuery({ name: 'tag', required: false, example: 'Logistics' })
  async getBlogPosts(@Query('category') category?: string, @Query('tag') tag?: string) {
  ...
  @Get('careers')
  @ApiOperation({ summary: 'Get active job openings with optional department filter' })
  ...
  @Post('careers/apply')
  @HttpCode(HttpStatus.CREATED)
  @ApiOperation({ summary: 'Submit job application and resume for an active posting' })
  ...
  @Get('seo/metadata')
  @ApiOperation({ summary: 'Get route-specific SEO metadata and JSON-LD schema' })
  ...
  @HttpCode(HttpStatus.CREATED)
  @ApiOperation({ summary: 'Track public pageview or interaction telemetry event' })
  async trackAnalyticsEvent(
    @Body() dto: TrackAnalyticsEventDto,
    @Ip() ipAddress: string,
    @Headers('user-agent') userAgent?: string
  ) {
    return this.marketingService.trackAnalyticsEvent(dto, ipAddress || '127.0.0.1', userAgent);
  }
}
